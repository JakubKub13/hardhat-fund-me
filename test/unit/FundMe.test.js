...
describe("FundMe", async function () {
    ...
    beforeEach(async function () {
        ...
        //const accounts = await ethers.getSigners() // returns whatever is in accounts section
        //const accountZero = accounts[0]
        deployer = (await getNamedAccounts()).deployer
        await deployments.fixture(["all"]) // we deploy all of our contracts
        fundMe = await ethers.getContract("FundMe", deployer) // get most recent contract we tell it
    ...
    describe("withdraw", async function () {
        ...
        it("Can withdraw ETH from a single founder", async function () {
            ...
            const { gasUsed, effectiveGasPrice } = transactionReceipt // {} pull out objects from another objects
            ...
            assert.equal(startingFundMeBalance.add(startingDeployerBalance), endingDeployerBalance.add(gasCost).toString()) //use .add because we works with big numbers
        })
